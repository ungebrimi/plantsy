...
const Avatar = ({
  ...
  async function updateProfileThumbnail(image: Tables<"files">) {
    ...
    try {
      ...
      console.error(error);
      addError("There was an issue updating your profile picture: " + error.message)
    }
  }
  ...
  const handleImageChange = async (
    ...
    try {
      ...
      if(formData.profile_picture && typeof formData.profile_picture === "object" && "id" in formData.profile_picture) {
        if (event.target.files && event.target?.files[0].name === formData.profile_picture.name) {
          addError("Same image name. if you still wish to upload the image, rename it and try again")
          return;
        }
  ...
  return (
    <div className="col-span-full">
      ...
      >
        Photo {" "}
        <span className={classNames(loading ? "animate-pulse" : "", "text-gray-500 italic")}>{formData.profile_picture ? `(${formData.profile_picture.name})` : ""} {loading && "(loading)"}</span>
      </label>
